	...
	for (int i = 0; i < this->width; i++) {
		for (int j = 0; j < this->height; j++) {
			...
			if (status == 1) {
				...
				this->board[i][j] = Cell(i, j, "WALL"); //временное решение без стен, изменить на WALL, для рандомной генерации
			}
		}
	}
}
		...
		if (cell.getEntity() != nullptr) {
			if ((typeid((*cell.getEntity())).name() == typeid(Character).name())) {
				if (board[x][y].getEntity() != nullptr) {
					if (typeid(*board[x][y].getEntity()).name() != typeid(Goblin).name() && typeid(*board[x][y].getEntity()).name() != typeid(Ogr).name() && typeid(*board[x][y].getEntity()).name() != typeid(Troll).name()) { // попробовать изменить на enemy
						if (typeid(*board[x][y].getEntity()).name() == typeid(Heal).name() || typeid(*board[x][y].getEntity()).name() == typeid(Protect).name() || typeid(*board[x][y].getEntity()).name() == typeid(Damage).name()) {
							(dynamic_cast<Character&>(*hero->getEntity())).takeItem(board[x][y].getEntity()); // возможно кастануть не надо
